...
| --- | --- | --- | --- | --- | --- | --- | --- |
| **L1-01** | Provider configuration readiness (Apify-only first) | Cyril | Coolify access available; builder governance committed | None (Cyril enters Coolify settings directly for Apify only; Outscraper excluded until separately approved); Gemini has no secret access; discovery stays disabled | `ai-security-governance`, `platform-operations` | Any request to handle credentials, tokens, or enable discovery | Apify settings verified present in Coolify while Outscraper remains unconfigured/excluded and `SYSTEM1_DISCOVERY_ENABLED=false` remains set |
| **L1-02** | Dashboard estimate and approval record (Apify-only first) | Cyril | L1-01 complete; Apify dashboard accessible | None (Cyril records exact displayed Apify amount and approves it; Outscraper excluded until separately approved); Gemini does not infer a price or trigger a run | `model-operations`, `ai-security-governance` | Any attempt to estimate without dashboard evidence or trigger a run | Written approval record with exact Apify dashboard estimate within USD 1.40 daily cap (Outscraper excluded) |
